...
router.post("/import", async (req, res) => {
	...
	const insertQuery = `INSERT INTO ${
		req.body.db_conf_table_name
	} (${Object.keys(mappings).join(", ")}) VALUES ${data
		.map(
			(data) =>
				`(${Object.values(mappings)
					.map((mappedColumn) => `'${data[mappedColumn]}'`)
					.join(", ")})`
		)
		.join(", ")};`;
	...
	connection = await mysql.createConnection({
		host:
			req.body.db_conf_table_host == "localhost"
				? "127.0.0.1"
				: req.body.db_conf_table_host,
