...
$("#wizard").steps({
    ...
    onStepChanging: async function (event, currentIndex, newIndex) {
         ...
         if(newIndex == 2){
            ...
            let res = await $.post("/connection",{hostname,username,password,database})
            if(res.status){
            ...
            if(checkedAction == "Import"){
                ...
                let res = await $.post("/import",{columns : {...columnMapping}, pathToFile,db_conf_table_name : $('#selectTable').val(), db_conf_table_host:  $("#hostname").val(),db_conf_table_user:  $("#username").val(),db_conf_table_pass:  $("#password").val(),db_conf_table_db:  $("#database").val(),})
                if(res.status){
                    document.getElementById("step5Heading").innerText = "Completed"
                    document.getElementById("step5Status").innerText = "Imported Successfully"
                ...
                try {
                    let response = await $.post("/export",{table : $('#selectTable').val(), host:  $("#hostname").val(),user:  $("#username").val(),pass:  $("#password").val(),db:  $("#database").val(),})
                    ...
                    document.getElementById("step5Heading").innerText = "Completed"
                    document.getElementById("step5Status").innerText = "Exported Successfully. The file should be downloading any minute now."
    ...
    onCanceled: function (event) {console.log(currentIndex, "onCancelled") },
    onFinishing: function (event, currentIndex) { console.log(currentIndex, "on Finishing"); return true }, 
    onFinished: function (event, currentIndex) {console.log(currentIndex, "on Finished") },
});
